...
(defn run-election
  ...
  Examples:
    ...
    (def leader (run-election treeherd \"/election\" candidate))
    @leader
...
"
  ([client election-node local-candidate]
     (let [mutex (Object.)
           ...
           get-leader (fn [candidates] (-> candidates first first))
           leader-ref (ref nil)]
       (future
         (locking mutex
           (loop [candidates (tc/children client election-node :watcher watcher)]
             (if (seq candidates)
               (do
                 (dosync (alter leader-ref (fn [_] (get-leader (sort-candidates election-node local-candidate candidates)))))
                 (.wait mutex)
