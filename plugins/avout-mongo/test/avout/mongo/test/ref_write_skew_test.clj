(ns avout.mongo.test.ref-write-skew-test
  (:use avout.core
        avout.mongo
        clojure.test)
...
(deftest mongo-ref-write-skew
  (println "running zk-ref-write-skew test...")
  (let [run-count 100
        max-threads 10
        client (connect "127.0.0.1")
        mongo-conn (mongo/make-connection "statedb" :host "127.0.0.1" :port 27017)
        c (mongo-ref client mongo-conn "/c-test" 0)
        d (mongo-ref client mongo-conn "/d-test" [])
        test-results (atom [])
